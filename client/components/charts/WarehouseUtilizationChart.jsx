...
const WarehouseUtilizationChart = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await statisticsAPI.getWarehouseUtilization();
            if (response.success) {
                setData(response.data || []);
            } else {
                showError('Failed to fetch warehouse utilization: ' + response.message);
            }
        } catch (err) {
            showError('Error fetching warehouse utilization: ' + (err.response?.data?.error || err.message));
        } finally {
            setLoading(false);
        }
    };
